...
cexRow = 0.2,# 0.2 + 1/log10(max(nr,2)),
cexCol = 0.2,# 0.2 + 1/log10(max(nc,2)),
cex.main = 1,
labRow = NULL,
labCol = NULL,
key = TRUE,
...
...
    ...
    # Check margins for type and length    
    if (!is.numeric(margins) || length(margins) != 2)
        stop("`margins' must be a numeric vector of length 2")

    # Set default cell note
    if (missing(cellnote))
        cellnote <- matrix("", ncol = ncol(x), nrow = nrow(x))

    # Check the Column dendrogram
    ...
    # Rearrange matrix according to dendrograms
    x <- x[rowInd, colInd]
    x.unscaled <- x
    # Also rearrange the cell notes
    ...
    # Set layout height if not given
    if (missing(lhei) || is.null(lhei))
        lhei <- c(keysize, 4)

    # Set layout width if not given
    ...
    if (missing(lmat) || is.null(lmat)) {
        ...
        if (!missing(ColSideColors)) {
            ...
            lmat <- cbind( lmat[,1], lmat[,2], c(lmat[1,1]+1, 0, 0))
        ...
        if (!missing(RowSideColors)) {
            ...
            lmat <- cbind(lmat[, 1] + 1, c(rep(NA, nrow(lmat) - 1), 1), lmat[,2] + 1, c(lmat[1,3]+1,lmat[2,3],lmat[3,3]))
            side_width = min(side.height.fraction*ncol(RowSideColors), 1)
            lwid <- c(lwid[1], side_width, lwid[2],1)
        }
        lmat[is.na(lmat)] <- 0
    }
    ...
    {
      plot.new()
      legend( "topright", unique( names( ColSideColors )), fill = unique( as.vector( ColSideColors ) ), title = "Sample Groups" )
    }
...
####################
# Borrowing the following from gplots
# (gplots isn't compatible with R 3.0 (yet), and so bypassing it for now).
####################
